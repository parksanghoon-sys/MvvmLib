        ...
        protected void AddSwDetailItem(FileCompareModel input, FileCompareModel output)
        {
            ...
            if (input is not null && output is not null)
            {
                detailedItem.을Content = $"""
                            표 24 서버 정보융합(운용통제용) 원본(소스)파일 목록 순번 {input.FileIndex} 파일명 {input.FileName}버전 2 크기 {input.FileSize}체크섬 {"0x" + input.Checksum}생성일자 {input.CreateDate} 라인수 {input.LineCount} 기능설명 
                            """;
                detailedItem.으로Content = $"""
                            표 24 서버 정보융합(운용통제용) 원본(소스)파일 목록 순번 {output.FileIndex} 파일명 {output.FileName}버전 3 크기 {output.FileSize}체크섬 {"0x" + output.Checksum}생성일자 {output.CreateDate} 라인수 {output.LineCount} 기능설명 
                            """;
